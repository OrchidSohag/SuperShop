                    ...
                    <div class="page-header float-right">
                        <div class="page-title">
                            <ol class="breadcrumb text-right">
                                ...
                                <li class="active">Products CRUD</li>
                            </ol>
                        </div>
                    </div>
                </div>
...
@section('content')
    <div class="container" >
        ...
        <nav  class="navbar navbar-center navbar-expand-lg navbar-light bg-light">
                <form method="post" action="{{route('SearchProduct')}}" class="form-inline my-2 my-lg-0">
                    {{--<input class="form-control mr-sm-2" type="search" placeholder="Search">--}}
                    @csrf
                    <select name="id">
                        <option selected>Product ID</option>
                        @foreach($id as $idd)
                            <option name = "id">{{$idd->product_id}}</option>
                        @endforeach
                    </select>
                ...
                <form method="post" action="{{route('DeleteProduct')}}" class="form-inline my-2 my-lg-0">
                    {{--<input class="form-control mr-sm-2" type="search" placeholder="Search">--}}
                    @csrf
                    <select name="id">
                        <option selected>Product ID</option>
